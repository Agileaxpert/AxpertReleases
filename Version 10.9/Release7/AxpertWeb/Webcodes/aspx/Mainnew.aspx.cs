...
public partial class aspx_Mainnew : System.Web.UI.Page
{
    ...
    //string executionTraceTimeInt = string.Empty;
    ...
    public string oldappurl = "false";
    public string oauLandingPage = string.Empty;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.Form["hdnAxGKey"] != null || (Request.Form["hdnAxGKey"] == null && Session["AxInternalRefresh"] != null && Session["AxInternalRefresh"].ToString() == "true"))
        {
            if (Request.Form["hdnAxGKey"] != null)
            {
                ...
                if (helperString != string.Empty)
                {
                    ...
                    Session["oldappurlfromnew"] = oldappurl = loginHelper.oldappurl == null ? oldappurl : loginHelper.oldappurl;
                    Session["oauLandingPage"] = oauLandingPage = loginHelper.oaulandingpage == null ? "" : loginHelper.oaulandingpage;
                }
                else
                {
                    SessExpires();
            ...
            else
            {
                ...
                if (Session["oldappurlfromnew"] != null)
                    oldappurl = Session["oldappurlfromnew"].ToString();
                if (Session["oauLandingPage"] != null)
                    oauLandingPage = Session["oauLandingPage"].ToString();
            }
        }
        else
        {
        ...
        else
        {
            ...
            if (Session["project"] != null)
            {
                ...
                {
                    string AppNameParams = "";
                    string appnamebeforeParams = "";
                    string[] ProjTitleList = appName.Split(':');
                    appnamebeforeParams = ProjTitleList[0];
                    for (int i = 1; i < ProjTitleList.Length; i++)
                    {
                        bool isValid = IsStringValid(ProjTitleList[i]);
                        if (isValid)
                        {
                            if (Session[ProjTitleList[i]] != null && Session[ProjTitleList[i]].ToString() != "")
                                ProjTitleList[i] = Session[ProjTitleList[i]].ToString();
                            // Console.WriteLine("The string is valid.");
                        }
                        else
                        {
                            char invalidCharacter = GetFirstInvalidCharacter(ProjTitleList[i]);
                            int index = ProjTitleList[i].IndexOf(invalidCharacter);
                            string invalidChars = "";
                            if (index != -1)
                            {
                                // Extract the substring before the character
                                invalidChars = ProjTitleList[i].Substring(index - 1 + 1);
                            }
                            ProjTitleList[i] = ProjTitleList[i].Split(' ')[0];
                            string sanitizedString = SanitizeString(ProjTitleList[i]);
                            if (Session[sanitizedString] != null && Session[sanitizedString].ToString() != "")
                                ProjTitleList[i] = Session[sanitizedString].ToString() + invalidChars;
                        }

                        AppNameParams += ProjTitleList[i];
                    }

                    AppNameParams = appnamebeforeParams + AppNameParams;
                    if (AppNameParams != "" && AppNameParams != String.Empty)
                        appName = AppNameParams;
                }
    ...
    {
        // Regular expression pattern to match letters, digits, and underscore
    ...
    static string SanitizeString(string input)
    {
        ...
        string sanitizedString = Regex.Replace(input, pattern, "");

        return sanitizedString;
    }

    static char GetFirstInvalidCharacter(string input)
    {
        // Regular expression pattern to match characters other than allowed ones
        string pattern = @"[^a-zA-Z0-9_]";
    ...
    private void GetUserOptions()
    {
        ...
        if (Session["axUserOptions"] == null || axUserOptions == string.Empty)// (Session["axUserOptions"] != null && Session["axUserOptions"].ToString() != "cached"))
        {
            ...
            if (Convert.ToString(Session["Build"]) == "F")
                ...
                UserOptions.Add("AppConfig", "\"display\":\"block\"♠\"onclick\":\"LoadIframe('tstruct.aspx?transid=axstc')\"♠\"title\":\"Developer Options\"");
            ...
            if (Convert.ToString(Session["Build"]) == "F")
                ...
                UserOptions.Add("Dashboard", "\"display\":\"block\"♠\"onclick\":\"nodeApi ? ToogleLeftMenu('dashboardPanel', 'true','Dashboard','') : showAlertDialog('error', 1000, 'client')\"♠\"title\":\"Dashboard\"");
    ...
    //private void SetExecutionTimeInt()
    //{
    //    executionTraceTimeInt = util.GetAdvConfigs("execution trace time interval");
    ...
    private void SetLandingPage()
    {
        ...
        hdHomeUrl.Value = Navigationpage;
        if (Session["oauLandingPage"] != null && Session["oauLandingPage"].ToString() != string.Empty)
        {
            string _oauOpenPage = Session["oauLandingPage"].ToString();
            char firstCharacter = _oauOpenPage[0];
            _oauOpenPage = _oauOpenPage.Substring(1);
            if (firstCharacter == 't')
                Navigationpage = "../aspx/tstruct.aspx?transid=" + _oauOpenPage;
            else if (firstCharacter == 'i')
                Navigationpage = "../aspx/iview.aspx?ivname=" + _oauOpenPage;
            else if (firstCharacter == 'p')
            {
                if (_oauOpenPage.StartsWith("HP"))
                    _oauOpenPage = _oauOpenPage.Replace("HP", "");
                Navigationpage = "../aspx/htmlpages.aspx?load=" + _oauOpenPage;
            }
            hdHomeUrl.Value = Navigationpage;
        }
    }
        ...
        try
        {
            ...
            if (HttpContext.Current.Session["user"] != null)
            {
                ...
                if (callService)
                {
                    ...
                    if (HttpContext.Current.Session["oldappurlfromnew"] != null && HttpContext.Current.Session["oldappurlfromnew"].ToString() == "true")
                        query = Constants.GET_SEARCH_DATA_OLDAPPURL.Replace("$KEYWORD$", srchKey.ToLower());
                    else
    ...
    #region global search
    public static DataTable GetMenuOldApp()
    {
        string iXml = string.Empty;
        string errlog = string.Empty;
        string fileName = string.Empty;
        ...
        try
        {
            ...
            if (fObj != null)
            {
                string _dbtype = HttpContext.Current.Session["axdb"].ToString();
                if (_dbtype.ToLower() == "postgresql" || _dbtype.ToLower() == "postgre")
                {
                    string _databaseName = HttpContext.Current.Session["dbDatabaseName"].ToString();
                    result = fObj.StringFromRedisOldUri(objUtil.GetRedisServerkey(fdKeyMenuData, "Menu"), schemaName, _databaseName);
                }
                else
                    result = fObj.StringFromRedis(objUtil.GetRedisServerkey(fdKeyMenuData, "Menu"), schemaName);
            }
        ...
        try
        {
            ...
            foreach (XmlNode menuchldNode in parentNodes)
            {
                ...
                if (!string.IsNullOrEmpty(hltype) && !string.IsNullOrEmpty(oldappurl))
                {
                    structname = hltype.Split('=')[1];
                    hltype = hltype.Split('.')[0];
                    dt = AddRowOldApp(dt, searchtext, hltype, structname, strparams, oldappurl);
                }
                XmlNodeList zeroChldNode = default(XmlNodeList);
                zeroChldNode = menuchldNode.ChildNodes;
                foreach (XmlNode firstChildNodes in zeroChldNode)
    ...
    private static DataTable AddRowOldApp(DataTable dt, string parent, string child, string target, string level, string oldappurl)
    {
        DataRow dr = dt.NewRow();
        dr["SEARCHTEXT"] = parent;
        dr["HLTYPE"] = child;
    ...
    private void GetMenuData()
    {
        ...
        try
        {
            ...
            if (result1 == string.Empty)
            {
                ...
                requestProcess_logtime += result1.Split('♠')[0];
    ...
    private void GetMenuDataOldUrl()
    {
        string result1 = string.Empty;
        string err = string.Empty;
        string lang_at = "";
        ...
        try
        {
            ...
            if (fObj != null)
            {
                ...
                {
                    string _databaseName = HttpContext.Current.Session["dbDatabaseName"].ToString();
                    result1 = fObj.StringFromRedisOldUri(util.GetRedisServerkey(fdKeyMenuData, "Menu"), schemaName, _databaseName);
                }
                else
                    result1 = fObj.StringFromRedis(util.GetRedisServerkey(fdKeyMenuData, "Menu"), schemaName);
            }
        ...
        {
            logobj.CreateLog("GetMenuDataOldUrl -- " + ex.Message, HttpContext.Current.Session["nsessionid"].ToString(), "GetMenuDataOldUrl-exception", "new");
        }
