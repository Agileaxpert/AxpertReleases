...
public partial class aspx_ImportNew : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            string isIframe = Request.Headers["Sec-Fetch-Dest"];
            if (!string.Equals(isIframe, "iframe", StringComparison.OrdinalIgnoreCase))
            {
                SessExpires();
                return;
            }
    ...
    protected void btnImport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (axpConfigStr != null && axpConfigStr.Rows.Count > 0)
                webserviceTimeout = Convert.ToInt32(axpConfigStr.AsEnumerable().Where(x => x.Field<string>("PROPS").ToLower() == "webservice timeout").Select(x => x.Field<string>("PROPSVAL")).FirstOrDefault());
    ...
    public DataTable exceldt(string filePath)
    {
        ...
        if (!string.IsNullOrEmpty(sSheetName))
        {
            ...
            // Load only the first 10 rows manually
            bool schemaLoaded = false;
            int rowCount = 0;
            while (oleExcelReader.Read() && rowCount < 10)
            {
                if (!schemaLoaded)
                {
                    for (int i = 0; i < oleExcelReader.FieldCount; i++)
                    {
                        dt.Columns.Add(oleExcelReader.GetName(i), oleExcelReader.GetFieldType(i));
                    }
                    schemaLoaded = true;
                }
                ...
                for (int i = 0; i < oleExcelReader.FieldCount; i++)
                {
                    newRow[i] = oleExcelReader.IsDBNull(i) ? null : oleExcelReader.GetValue(i);
                }
                dt.Rows.Add(newRow);
                rowCount++;
            }
