...
public partial class Tstruct : System.Web.UI.Page
{
    ...
    private void CreateToolbarButtons(TStructDef strObj)
    {
        ...
        for (int i = strObj.btns.Count - 1; i >= 0; i--)
        {
            ...
            switch (task)
            {
                ...
                case "listview":
                    if ((ConfigurationManager.AppSettings["AxOldModel"] != null && ConfigurationManager.AppSettings["AxOldModel"].ToString() == "true") || (Session["IsAxOldDataList"] != null && Session["IsAxOldDataList"].ToString() == "true"))
                    {
    ...
    public static string GetFormLoadValues(string key, string tstQureystr, string isDraft, string forceRefresh, string isDupTab, string isTstHtmlLs, string isFromDummytrue = "false")
    {
        ...
        try
        {
            ...
            if ((isTstParamLoad == "true" || (isTstParamLoad == "false" && strObj.wsPerfFormLoadCall) || iswsPerfFGDcName) && isDraft != "true")
            {
