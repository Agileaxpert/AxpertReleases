...
public class FDR
{
    ...
    public string StringFromRedisOldUri(string key, string schemaName = "", string dbName = "")
    {
        ...
        try
        {
            ...
            if (redis.IsConnected)
            {
                if (dbName == "")
                    ...
                    key = schemaName == "" ? schemaNameKey + "~" + dbName + '-' + key : schemaName + "~" + dbName + '-' + key;
                bytes = (byte[])cacheClient.StringGet(key);
        ...
        {
            logObj.CreateLog("Redis Server Function(StringFromRedisOldUri), Message:" + ex.Message, GetSessionId(), "RedisServerolduri", "new");
        }
        finally
        {
            RedisClose(redis);
    ...
    public ArrayList GetPrefixedKeys(string strPrefix, bool prefixWithSchema = false)
    {
            ...
            if (redis != null && redis.IsConnected)
            {
                ...
                foreach (var endpoint in endpoints)
                {
                    ...
                    if (server.IsConnected)
                    {
                        ...
                        {
                            foreach (string key in server.Keys(pattern: keyForPattern + strPrefix + "-*"))
                            {
                                if (key.Contains(keyForPattern + "General" + "-"))
                                    list.Add(key);
                            }
                        }
                        else if (keyForPattern != "")
                        {
                            foreach (string key in server.Keys(pattern: keyForPattern + strPrefix + "-*"))
                            {
                                if (key.Contains(keyForPattern + strPrefix))
                                    list.Add(key);
                        ...
                        {
                            foreach (string key in server.Keys(pattern: "*-" + strPrefix + "-*"))
                            {
                                if (key.Contains(keyForPattern + strPrefix))
                                    list.Add(key);
                            }
