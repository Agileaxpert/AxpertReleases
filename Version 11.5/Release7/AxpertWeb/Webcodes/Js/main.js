    ...
    if (doPageUnload != "false") {
        try {
            ...
            if (!localSessionVal || localSessionVal == 0) {
                ...
                clearLocalStorage(['projInfo-', 'versionInfo-', 'langInfo-', 'hybridGUID-', 'compressedMode-', 'duplicateUser-', 'instanceName-'], true);
                window.localStorage.removeItem("axpertLocalsession");
                if(isExeSave)
...
function LoadIframe(src,isautoprocess=false) {
    ...
    function searchLoadIframe(type, src) {
        ...
        if (type.toLowerCase() == "iview" || type.toLowerCase() == "iviewinteractive") {
            if (srcDtls[1] == "null")
                loadurl = 'ivtoivload.aspx?ivname=' + srcDtls[0];
            else {
            ...
            if (srcDtls[1] != null && srcDtls[1] != undefined && srcDtls[1].split('~').length > 1)
                window.open('help.aspx?' + srcDtls[1].split('~')[1], '_blank');
        }
        if (loadurl != "") {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (typeof OldAppUrl != "undefined" && OldAppUrl == "true") {
                                gbSearch = GetGLoSrchItemsFromOldapp(tblSearchData);
...
function GetGLoSrchItemsFromOldapp(tblSearchData) {
    var gbSearch = [];
    var splitChar = "~";
    $(tblSearchData.Table).each(function (iIndex, sElement) {
        // if (sElement.SEARCHTEXT != null && sElement.SEARCHTEXT.toLowerCase().indexOf(txtInput) >= 0) {
        if (getCaseInSensitiveJsonProperty(sElement, "SEARCHTEXT") != null) {
            ...
            gbSearch.push(getCaseInSensitiveJsonProperty(sElement, "SEARCHTEXT") + "♣" + getCaseInSensitiveJsonProperty(sElement, "HLTYPE") + "♦" + getCaseInSensitiveJsonProperty(sElement, "STRUCTNAME") + "*♠*" + newParams + "♣" + getCaseInSensitiveJsonProperty(sElement, "SEARCHTEXT"));
        }
    });
    return gbSearch;
}
    ...
    function SessionExpireAlert(setTime, firstCall = true) {
        ...
        function KeepSessionAlive(serSessATime) {
            if (serSessATime != "") {
                ...
                if (serSessTime > 120000)
                    serSessATime = serSessATime - 120000;
                else
                    serSessATime = serSessATime - 60000;
            }
            if (creatSATimeOut) {
        ...
        function showWorkBench() {
            doPageUnload = "false";
            window.location.replace('../../aspx/dwb.aspx')
        }
...
if (topMenuHTML === "") {
    ...
    if (childs && childs.length) {
        ...
        return `${menuConfiguration.menuTemplete.functionalLi.opener}
                ${menuConfiguration.menuTemplete.functionalLi.anchorGenerator(childObj)}
...
function resetSplitter(calledFrom) {
    ...
    if (calledFrom == "horizantal") {
        ...
        $('.panel-fisrt-part, .panel-second-part').css({ width: '50%', height: '100%' });
    }
}
...
function OpenOnPropertyBase(UrlToOpen) {
    ...
    GetProcessTime();
    $("#axpiframe").attr('src', UrlToOpen+"&hdnbElapsTime="+ callParentNew("browserElapsTime"));
}
