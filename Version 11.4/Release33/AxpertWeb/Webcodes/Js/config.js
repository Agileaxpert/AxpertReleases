...
$j(document).ready(function () {
    ...
    if (applstJson != "") {
        ...
        if (typeof fstValuearm != "undefined") {
            ...
            if (appSettingList && appSettingList.appsettings) {
                if (appSettingList.appsettings.hasOwnProperty(projfile) && typeof appSettingList.appsettings[projfile].ExternalResources != "undefined") {
                    let projData = appSettingList.appsettings[projfile];
                    document.getElementById("txtRMQAPIURL").value = projData.ExternalResources.AxRMQAPIURL || "";
                    document.getElementById("txtSignalRapiURL").value = projData.ExternalResources.AxSignalRapiURL || "";
                    document.getElementById("txtFCMSendMsgURL").value = projData.ExternalResources.AxFCMSendMsgURL || "";
                    document.getElementById("txtRapidSaveURL").value = projData.ExternalResources.AxRapidSaveURL || "";
                    document.getElementById("txtpegemailactionurl").value = projData.ExternalResources.axpegemailactionurl || "";
                    document.getElementById("txtScriptsAPIURL").value = projData.ExternalResources.AxScriptsAPIURL || "";
                }
    ...
    if (applstJson != "") {
        var RMqValuearm = $("#RMQueueproj option:first").val();
        if (typeof RMqValuearm != "undefined") {
            $("#RMQueueproj").val(RMqValuearm).focus();
            $('#RMQueueproj').trigger('change');
            var proj = RMqValuearm;
            if (proj.indexOf("\\") != -1)
                proj = proj.split("\\")[0];
            $("#hdnRMQueueproj").val(proj);
            if (appSettingList && typeof appSettingList === "string") {
                appSettingList = JSON.parse(appSettingList);
            }
            if (appSettingList && appSettingList.appsettings) {
                if (appSettingList.appsettings.hasOwnProperty(projfile) && typeof appSettingList.appsettings[projfile].RMQueue != "undefined") {
                    let projData = appSettingList.appsettings[projfile];
                    document.getElementById("txtRmQueueHost").value = projData.RMQueue.RMQueueHost || "";
                    document.getElementById("txtRmQueuePort").value = projData.RMQueue.RMQueuePort || "";
                    document.getElementById("txtrmqueueuser").value = projData.RMQueue.RMQueueUser || "";
                    document.getElementById("txtrmqueuepwd").value = projData.RMQueue.RMQueuePassword || "";
        ...
        try {
            ...
            if (appSettingList && appSettingList.appsettings) {
                ...
                if (appSettingList.appsettings.hasOwnProperty(proj) && typeof appSettingList.appsettings[proj].ARM != "undefined") {
                    ...
                    if (appSettingList.appsettings.hasOwnProperty(proj) && typeof appSettingList.appsettings[proj].ExternalResources != "undefined") {
                        document.getElementById("txtRMQAPIURL").value = projData.ExternalResources.AxRMQAPIURL || "";
                        document.getElementById("txtSignalRapiURL").value = projData.ExternalResources.AxSignalRapiURL || "";
                        document.getElementById("txtFCMSendMsgURL").value = projData.ExternalResources.AxFCMSendMsgURL || "";
                        document.getElementById("txtRapidSaveURL").value = projData.ExternalResources.AxRapidSaveURL || "";
...
$j(document).on("change", "#RMQueueproj", function (e) {
    var lvalue = $(this).val();
    if (applstJson != "") {
        var proj = lvalue;
        if (proj.indexOf("\\") != -1)
        ...
        try {
            ...
            if (appSettingList && appSettingList.appsettings) {
                isArm = "true"
                if (appSettingList.appsettings.hasOwnProperty(proj) && typeof appSettingList.appsettings[proj].RMQueue != "undefined") {
                    let projData = appSettingList.appsettings[proj];
                    document.getElementById("txtRmQueueHost").value = projData.RMQueue.RMQueueHost || "";
                    document.getElementById("txtRmQueuePort").value = projData.RMQueue.RMQueuePort || "";
                    document.getElementById("txtrmqueueuser").value = projData.RMQueue.RMQueueUser || "";
...
function RMQueuesaveWs() {
    ...
    $.ajax({
        ...
        cache: false,
        async: true,
        data: JSON.stringify({proj: proj, aRMQHost: $("#txtRmQueueHost").val(), aRMQPort: $("#txtRmQueuePort").val(), aRMQUser: $("#txtrmqueueuser").val(), aRMQPwd: $("#txtrmqueuepwd").val()
        }),
        dataType: 'json',
        contentType: "application/json",
        success: function (data) {
