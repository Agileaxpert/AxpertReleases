...
public partial class AxpertAdmin : System.Web.UI.Page
{
    ...
    public static string ARMConnection(string aKey, string aUrl, string aScriptsUrl, string aPeg, string proj, string aExpiryMinutes, string aClientSSO)
    {
        ...
        try
        {
            ...
            var externalResources = new Dictionary<string, object>
            {
            ...
            var externalResourcesData = new Dictionary<string, object> {
                { "ExternalResources", externalResources }
            };
            var externalResourcesDict = new Dictionary<string, object>
            {
                { project, externalResourcesData }
            };
            var externalResourcesmainDict = new Dictionary<string, object>
            {
                { "appsettings", externalResourcesDict }
            };
            string _jsonString = JsonConvert.SerializeObject(externalResourcesmainDict);
            if (!Filefi.Exists)
            {
    ...
    [WebMethod]
    public static string RMQueueConnection(string proj, string aRMQHost, string aRMQPort, string aRMQUser, string aRMQPwd)
    {
        string result = "";
        try
        {
    ...
    [WebMethod]
    public static string DelRMQueueConnectionWs(string proj)
    {
        string result = "";
        try
        {
            ...
            if (Directory.Exists(directoryPath))
            {
                ...
                {
                    JObject projectObj = (JObject)jsonARM["appsettings"][proj];
                    if (projectObj["RMQueue"] != null)
                    {
                        projectObj.Property("RMQueue").Remove();
                        string modifiedJsonARM = jsonARM.ToString();
                        File.WriteAllText(filePath, modifiedJsonARM);
                    }
                }
