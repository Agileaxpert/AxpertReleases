    ...
    public class Util
    {
        ...
        public string GetAxRMQSettings(string strProj)
        {
            ...
            if (strProj != string.Empty)
            {
                ...
                {
                    FDR fdrObj = new FDR();
                    jsoncontents = fdrObj.StringFromRedis(Constants.AXARM_RMQSettings, strProj);
                    if (jsoncontents == string.Empty)
                    {
                        FileInfo fi = new FileInfo(ScriptsPath + "\\AppSettings.ini");
                        if (fi.Exists)
                        {
                            ...
                            if (jsonARM["rmqsettings"] != null)
                            {
                                ...
                                FDW fdwObj = new FDW(strProj);
                                fdwObj.SaveInRedisServer(Constants.AXARM_RMQSettings, jsoncontents, Constants.AXARM_RMQSettings, strProj);
                            }
                            else
                            {
                                FDW fdwObj = new FDW(strProj);
                                fdwObj.SaveInRedisServer(Constants.AXARM_RMQSettings, "noRMQSettings", Constants.AXARM_RMQSettings, strProj);
                            }
                        }
                        else
                        {
                    ...
                    else if (jsoncontents != "noRMQSettings")
                    {
                        HttpContext.Current.Session["AxRabbitMQSet"] = jsoncontents;
                        return jsoncontents;
                    }
