...
public class TStructData
{
    ...
    public void DelHeaderAttaFromScriptFolder(string hdrfiles)
    {
        try
        {
            string sFileDir = scriptsPath + "Axpert\\" + sessionid + "\\";
            string[] hdrfileList = hdrfiles.Split(',');
            foreach (var hdelFile in hdrfileList)
            {
                sFileDir += hdelFile;
                try
                {
                    if (File.Exists(sFileDir))
                        File.Delete(sFileDir);
                }
                catch (Exception) { }
            }
        }
                ...
                if (msg != null)
                {
                    ...
                    if (saveResult.IndexOf(',') != -1)
                    {
                        ...
                        if (recId != string.Empty)
                        {
                            ...
                            if (!string.IsNullOrEmpty(filesCpy))
                            {
                                DelHeaderAttaFromScriptFolder(filesCpy);
                            }
                            try
                            {
            ...
            if (action != null)
            {
                ...
                if (actionValue.ToLower() == "save")
                {
                    ...
                    if (recId != string.Empty)
                    {
                        ...
                        if (!string.IsNullOrEmpty(files))
                        {
                            DelHeaderAttaFromScriptFolder(files);
                        }
                        //ClearIviewDataKey();
                        if (transid == "ad_pr")
    ...
    private void FieldToXML(string fldName, string rowNo, string value, string oldValue, string idvalue, string oldIdValue, string dcNo, Boolean fldSaveNormal, bool isSubGrid, string subRowNo, string calledFrom, TStructDef.FieldStruct fld = new TStructDef.FieldStruct())
    {
        ...
        if (recordid != "0")
        {
            if (value != oldValue)
            {
                ...
                {
                    //For these 2 transid ov value passing alwasy empty
                }
                else
                    oldVal = "ov='" + oldValue + "'";
            }
