...
function generateHTML(sheetNames, columns) {
    ...
    try {
        setTimeout(function () {
            setTimeout(function () {
                if ($("#selectUpdatePrimaryKey option[value!='']").length == 0) {
                    $("#selectUpdatePrimaryKey").val('Choose Primary Key').trigger("change");
                    $("#selectUpdatePrimaryKey").siblings(".select2").find(".select2-selection").css("height", "34px");
                }
            }, 0);
        }, 200);
    } catch (ex) { }
    ...
    $('#selectform').on('select2:select', function (e) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                var visibleTstFlds = datas.VisibleTstFlds.m_StringValue;
                var visibleTstFldsAllowDuplicate = datas.visibleTstFldsAllowDuplicate.m_StringValue;
                let strautoGenFlds = datas.TstfldAutoGenerate.m_StringValue;
                var separator = ',♠';
                ...
                var newStrAllowDup = "";
                if (separatorIndex !== -1) {
                    newStrAllowDup = visibleTstFldsAllowDuplicate.substring(0, separatorIndex); 
                }
                if (separatorIndex !== -1 && strautoGenFlds != null && strautoGenFlds != "") {
                    //autoGenFlds = strautoGenFlds.split(",");
                    $("#hdnAutoGenFlds").val(strautoGenFlds);
                } else {
                    $("#hdnAutoGenFlds").val("");
                }

                var UpdatePkeyArray = newStrAllowDup.split(",");
...
function DropzoneInitImport() {
    ...
    myDropzone.on("complete", function (file) {
        if (!$("#mulForm").is(":checked") && $("#selectform").length > 0 && $("#selectform").val() == null) {
...
$(document).ready(function () {
    ...
    $(document).on("change", "#dataupdateflagcheck", function () {
        if ($(this).is(":checked")) {
            ...
            $("#selectUpdatePrimaryKeyContainer").hide();
            //$("#selectUpdatePrimaryKey").val('').trigger("change");//Choose Primary Key"
            $("#selectUpdatePrimaryKey").val('Choose Primary Key').trigger("change");
        }
    });
